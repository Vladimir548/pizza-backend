...
constructor(private prisma:PrismaService,
  ...
  async create(dto: CartItemDto) {
		...
		const matchIngredients = existingItemsIngredients.find(item => JSON.stringify(item.ingredients.map(ingredient => ingredient.id).sort()) === JSON.stringify(ingredientsArray.sort()))
		if(matchIngredients){
 ...
 async remove(id: number,cartId:number,) {
	...
	async calcItemPrice (cartId:number){
		...
		for(const item of cartItems ) {
				const totalPriceIngredient = item.ingredients.reduce((acc,val) => Number(acc)+ Number(val),0)
				const totalPrice = (item.size.price + totalPriceIngredient) * item.quantity
				return totalPrice
		}
